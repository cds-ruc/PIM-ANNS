...
if [ $ALL_TESTS_SUCCESS -eq 0 ]; then
    ...
    echo "Some tests failed, please run exp1.sh again."
    exit 1
fi
...
output_file="$PROJECT_ROOT/AE/exp1.txt"
...
do
    ...
    if [ -f "$dpu_file" ]; then
        ...
        echo "SIFT, DPU, nprobe=$np, qps=$dpu_qps" >> "$output_file"
    fi

    if [ -f "$batch_dpu_file" ]; then
        batch_dpu_qps=$(extract_value "$batch_dpu_file" "qps")
        echo "SIFT, Batch DPU, nprobe=$np, qps=$batch_dpu_qps" >> "$output_file"
    fi

    if [ -f "$cpu_file" ]; then
        cpu_qps=$(extract_value "$cpu_file" "qps")
        echo "SIFT, CPU, nprobe=$np, qps=$cpu_qps" >> "$output_file"
    fi
done
...
do
    ...
    if [ -f "$dpu_file" ]; then
        dpu_qps=$(extract_value "$dpu_file" "qps")
        echo "SPACE, DPU, nprobe=$np, qps=$dpu_qps" >> "$output_file"
    fi

    if [ -f "$batch_dpu_file" ]; then
        batch_dpu_qps=$(extract_value "$batch_dpu_file" "qps")
        echo "SPACE, Batch DPU, nprobe=$np, qps=$batch_dpu_qps" >> "$output_file"
    fi

    if [ -f "$cpu_file" ]; then
        cpu_qps=$(extract_value "$cpu_file" "qps")
        echo "SPACE, CPU, nprobe=$np, qps=$cpu_qps" >> "$output_file"
    fi
done
